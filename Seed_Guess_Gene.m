...
global Node_i Node_i_child Tme_Seed Ctrl_No Active_Ind_Tran Active_Ind_Goal

if length(varargin)>1
    Node_i = varargin{1};
    Node_i_child = varargin{2};
    Seed_Obj = @Seed_Conf_Obj;
else
    Node_i = varargin{1};
    Node_i_child = Node_i;
    Seed_Obj = @Seed_Conf_Obj_Self;
end
...
[x_i_child,~,exitflag,output]= fmincon(Seed_Obj,x_i_child,[],[],[],[],RobotState_LowBd,RobotState_UppBd,@Seed_Conf_Constraint,Seed_Opt);
...
StateNDot_Seed = reshape(x_intp(:,2:end), length(C_q_qdot)*2*(Ctrl_No-1), 1);
Ctrl_Seed = reshape(u_array, length(u_i) *(Ctrl_No-1),1 );
Opt_Seed = [Tme_Seed; StateNDot_Seed; Ctrl_Seed];

Opt_Lowbd = 0.1;
Opt_Uppbd = inf; 

Opt_Lowbd = [Opt_Lowbd; repmat(RobotState_LowBd', [Ctrl_No-1,1]); repmat(Ctrl_LowBd, [Ctrl_No-1,1])];
Opt_Uppbd = [Opt_Uppbd; repmat(RobotState_UppBd', [Ctrl_No-1,1]); repmat(Ctrl_UppBd, [Ctrl_No-1,1])];
